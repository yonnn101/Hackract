...
import {
  ...
  FiSmartphone, FiMonitor, FiClock, FiEye, FiEyeOff,
  FiToggleLeft, FiToggleRight, FiAlertTriangle, FiCheck,
  FiCpu, FiKey, FiPlus, FiTrash2, FiSettings
} from "react-icons/fi";
...
// ── API Key credential vault storage (localStorage placeholder) ───────────────
const VAULT_STORAGE_KEY = "hackract_credential_vault";
const loadVault = () => {
  try {
...
const saveVault = (entries) => {
  try { localStorage.setItem(VAULT_STORAGE_KEY, JSON.stringify(entries)); } catch { /* */ }
};
...
const OrganizationSettings = () => {
  ...
  useEffect(() => {
    if (activeTab === "legal") fetchAgreements();
  }, [activeTab]);
  ...
  const handleChangePassword = async () => {
    ...
    setSaving(true);
    try {
      await api.patch("/users/change-password", { oldPassword: currentPassword, newPassword });
  ...
  const addVaultEntry = () => {
    ...
    const entry = { id: Date.now(), name: newKeyName.trim(), value: newKeyValue, addedAt: new Date().toISOString() };
    const next = [...vaultEntries, entry];
  ...
  const removeVaultEntry = (id) => {
    const next = vaultEntries.filter(e => e.id !== id);
    setVaultEntries(next);
    saveVault(next);
  ...
  const tabs = [
    ...
    { id: "workspace", label: "Workspace & AI", icon: FiCpu },
  ];
      ...
      <div className="max-w-[1000px] mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4 mb-2">
          <div className="w-10 h-10 rounded-xl bg-[#00c477]/10 flex items-center justify-center">
            <FiSettings className="text-[#00c477]" size={18} />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">Organization Settings</h1>
            <p className="text-xs text-gray-500">Manage account, security, preferences, and workspace</p>
          </div>
        </div>

        {/* TAB NAV */}
        <div className="flex gap-1 bg-[#0c0c0c] border border-white/5 rounded-xl p-1 flex-wrap">
          {tabs.map(tab => (
            <button
              key={tab.id}
        ...
        {activeTab === "account" && (
          <div className="space-y-6">
            ...
            <SectionCard title="Change Password" icon={FiLock} description="Update your login password">
              <FieldRow label="Current Password">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowCurrentPw(!showCurrentPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 cursor-pointer">
              ...
              <FieldRow label="New Password">
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowNewPw(!showNewPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 cursor-pointer">
              ...
              <SectionCard title="Two-Factor Authentication" icon={FiSmartphone} description="Add an extra layer of security" badge="Coming Soon">
                <div className="opacity-30 pointer-events-none space-y-4">
                  <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/5">
                    ...
                    <button className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[10px] text-gray-400 font-mono uppercase">Setup</button>
                  </div>
                </div>
              </SectionCard>
              <ComingSoonOverlay />
            ...
            <div className="relative">
              <SectionCard title="Active Sessions" icon={FiMonitor} description="Manage devices with access" badge="Coming Soon">
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/5">
                      ...
                      {s.current ? (
                        ...
                        <button className="px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20 text-[10px] text-red-400 font-mono uppercase">Revoke</button>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {activeTab === "notifications" && (
          <div className="space-y-6">
            ...
            <SectionCard title="Vulnerability Updates" icon={FiAlertTriangle} description="Alerts about security findings">
              ...
              <Toggle checked={notifPrefs.findingStatusChange} onChange={() => toggleNotif("findingStatusChange")} label="Finding status changes" />
            ...
            <SectionCard title="Agreement Center" icon={FiFileText} description="View your signed agreements and legal documents">
              {loadingAgreements ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-[10px] text-gray-600 mt-1">Agreements will appear here once signed</p>
                </div>
              ) : (
        ...
        {activeTab === "workspace" && (
          <div className="space-y-6">
            ...
            {/* Automation Preferences — Coming Soon */}
            <div className="relative">
              <SectionCard title="Automation Preferences" icon={FiCpu} description="Configure AI agent default behavior" badge="Coming Soon">
                <div className="opacity-30 pointer-events-none space-y-5">
                  <FieldRow label="Default Scan Intensity">
                    <select className="w-full border border-white/10 rounded-xl bg-white/[0.02] text-sm text-white px-4 py-2.5 font-mono focus:outline-none">
                      <option>Standard</option>
                      <option>Aggressive</option>
                      <option>Stealth</option>
                    </select>
                  </FieldRow>
                  <Toggle checked={false} onChange={() => {}} label="Auto-generate reports after scan completion" />
                  <Toggle checked={true} onChange={() => {}} label="Enable AI suggestions during workflow editing" />
                </div>
              </SectionCard>
              <ComingSoonOverlay />
            </div>
            ...
            {/* Credential Vault */}
            <SectionCard title="Credential Vault" icon={FiKey} description="Manage third-party API keys for scanning tools">
              <div className="space-y-3">
                {vaultEntries.map(entry => (
                  <div key={entry.id} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/5">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <FiKey className="text-[#00c477] shrink-0" size={16} />
                      <div className="min-w-0">
                        <p className="text-sm text-gray-200 font-medium">{entry.name}</p>
                        <p className="text-[10px] text-gray-500 font-mono truncate">
                          {showKeyValues[entry.id] ? entry.value : "•".repeat(Math.min(entry.value?.length || 20, 30))}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <button onClick={() => toggleShowKey(entry.id)} className="p-2 rounded-lg hover:bg-white/5 text-gray-500 hover:text-gray-300 transition-colors cursor-pointer">
                        {showKeyValues[entry.id] ? <FiEyeOff size={14} /> : <FiEye size={14} />}
                      </button>
                      <button onClick={() => removeVaultEntry(entry.id)} className="p-2 rounded-lg hover:bg-red-500/10 text-gray-500 hover:text-red-400 transition-colors cursor-pointer">
                        <FiTrash2 size={14} />
                      </button>
                    </div>
                  </div>
                ...
                {vaultEntries.length === 0 && (
                  <div className="text-center py-6">
                    <FiKey className="text-gray-600 mx-auto mb-3" size={28} />
                    <p className="text-sm text-gray-500">No API keys stored</p>
                    <p className="text-[10px] text-gray-600 mt-1">Add keys for GitHub, LLM providers, or other integrations</p>
                  </div>
                )}

                {/* Add new key */}
                <div className="border-t border-white/5 pt-5 space-y-3">
                  <p className="text-[10px] font-mono text-gray-500 uppercase tracking-widest">Add New Key</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <Input value={newKeyName} onChange={e => setNewKeyName(e.target.value)} placeholder="Key name (e.g. GitHub Token)" />
                    <Input value={newKeyValue} onChange={e => setNewKeyValue(e.target.value)} placeholder="API key value" type="password" />
                  </div>
                  <button
                    onClick={addVaultEntry}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs text-gray-300 font-medium hover:bg-white/10 transition-colors cursor-pointer"
                  >
                    <FiPlus size={14} />
                    Add API Key
                  </button>
